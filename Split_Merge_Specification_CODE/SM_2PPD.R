...
Testing2PPD_SM<- function(NSIM = 20000, 
                                        burn_in = 100, 
                                        thinning = 1,
                                        y, 
                                        prior_par, 
  ...
  SS <- runif(5,-.2,.2)
  mean_k         <- variance_k  <- sample( x = SS, size = n, replace = T )
  mean_k[zi==1]  <- m1 *sign(y[zi==1]) #*mean_k[zi==1]+runif(1)
  umk     <- unique(mean_k)
  
  for(q in 1:5){
    variance_k[which(mean_k==umk[q])] <- rinvgamma(1,a_0,b_0)
  }
  for(q in 5:7){
    variance_k[which(mean_k==umk[q])] <- 1
  }
  ...
  for(sim in 2:(NSIM*thinning + burn_in)){
    ...
    if(sim == burn_in+1){time3 <- Sys.time()}
...
#    if((sim%%500==0) & sim > burn_in) plot(apply(ZiCon[1:sim,],2,mean)~y) 
   ...
   #   if(sim %% 50 ==0 )  plot(apply(ZiCon[1:rr,],2,mean)~y)
      }
  ...
  out   <- list(ThetaCon = ThetaCon,
                ...
                tottimeElap=difftime(time2, time1, tz,
                                                                units = "secs"),
                MCMCtimeElap=difftime(time2, time3, tz,
                                      units = "secs"),
  ...
  class(out) <- "BNPtestingSM" #Code-name for our model
  return(out)
}
