...
Testing2PPD <- function(NSIM = 20000, 
                                        burn_in = 100, 
                                        thinning = 1,
                                        y, 
                                        prior_par, 
...
# m1 is estimated by data
  ...
  # kmc[(km$cluster==1)] <- 0
  # kmc[(km$cluster==2)] <- 1
  #zi[sy$ix] <- kmc # come vincolo a farlo partire sempre da zero?
  ...
  for(sim in 2:(NSIM*thinning + burn_in)){
    ...
    ################################## reorder the labels after theta have been updated
    if(sum(Data[,4]==0)>1){
  ...
  out <- list(ThetaCon = ThetaCon,
              ...
              m1Con = m1Con, SMc=SMc, tottimeElap=difftime(time2, time1, tz,
                                                        units = "secs"),
